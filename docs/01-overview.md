# 📘 Обзор проекта

`Rclone taskboard` это веб-слой управления для backup-процессов на базе `rclone`. Проект убирает связку shell-скриптов и systemd-юнитов из повседневной работы и переносит запуск задач, очереди, расписание и историю в одно приложение с API и dashboard.

---

## 🎯 Цели проекта

- Централизовать управление backup-задачами
- Разделить управление задачами и фактический перенос данных
- Поддержать запуск по расписанию и по событиям
- Сделать историю запусков и шагов наглядной
- Подготовить единый runtime для `docker` и `systemd`

---

## 🧱 Основные блоки

| Блок | Ответственность |
| --- | --- |
| FastAPI app | API, dashboard, конфигурация |
| Scheduler | Создание плановых запусков |
| Worker queues | Исполнение задач из очередей |
| Watcher | Передача filesystem events в API |
| SQLite | Хранение runs, steps, events и state; диагностика размера и WAL |

---

## 🧭 Модель исполнения

Проект использует простую схему:

- приложение решает, когда и что запускать
- `rclone` отвечает только за перенос данных
- задачи описываются в рабочем каталоге
- watcher работает внутри backend и больше не требует отдельного сервиса

---

## 📁 Ключевые пути

| Путь | Назначение |
| --- | --- |
| `taskboard/backend/app/` | Исходный код backend |
| `taskboard/backend/app/jobs/default_jobs.example.json` | Шаблон каталога задач |
| `taskboard/backend/app/jobs/default_jobs.json` | Рабочий каталог задач |
| `taskboard/docker-compose.yml` | Развертывание через Docker |
| `rclone-taskboard.service` | Unit-файл для запуска через systemd |

---

## ✅ Что даёт проект

- Dashboard для управления и просмотра состояния
- Ручной запуск профилей и отдельных задач
- Историю запусков с деталями шагов
- Видимость размера БД и состояния backend-процесса
- Автоматизацию по расписанию и по событиям
- Настройку каталога через UI и API

---

## 📌 Рабочий каталог

Основной рабочий файл:

`taskboard/backend/app/jobs/default_jobs.json`

Он создаётся из шаблона:

`taskboard/backend/app/jobs/default_jobs.example.json`

Шаблон хранится в Git, а рабочий файл создаётся уже в установленной системе.
